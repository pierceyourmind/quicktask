...
must_haves:
  ...
  key_links:
    ...
    - from: "QuickTask/Sources/Panel/PanelManager.swift"
      to: "QuickTask/Sources/Panel/FloatingPanel.swift"
    ...
    - from: "QuickTask/Sources/Panel/FloatingPanel.swift"
      to: "QuickTask/Sources/Views/ContentView.swift"
...
<task type="auto">
  ...
  <files>QuickTask/Sources/Panel/FloatingPanel.swift</files>
  <action>
...
Do NOT override `resignKey()` here — that will be added in Plan 03 for focus-return behavior.
  ...
  <verify>
Project compiles with `cd /home/rob/projects/todo-app/QuickTask && swift build`. FloatingPanel class exists with all required style mask flags, canBecomeKey override, and NSHostingView bridging.
  </verify>
  <done>
FloatingPanel.swift exists at QuickTask/Sources/Panel/FloatingPanel.swift with correct NSPanel subclass configuration: .nonactivatingPanel style mask, .floating level, canBecomeKey = true, canBecomeMain = false, NSHostingView content bridging, isReleasedWhenClosed = false.
  </done>
</task>
...
Extend the existing AppDelegate (created in Plan 01) to add the NSStatusItem:
